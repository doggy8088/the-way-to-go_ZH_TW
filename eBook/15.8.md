...
為進一步深入理解 `http` 包以及如何建構網頁伺服器功能，讓我們來學習和體會下面的例子：先列出程式碼，然後給出不同功能的實現方法，程式輸出顯示在表格中。
...
另外利用 `logger` 套件的函式，針對每個請求在伺服器端命令列列印日期、時間和 URL。

處理函式 | 呼叫 URL | 瀏覽器獲得響應
--------|----------|---------------
`HelloServer()` | http://localhost:12345/go/hello | hello, world!

包 `expvar` 可以建立（Int，Float 和 String 型別）變數，並將它們發佈為公共變數。它會在 HTTP URL `/debug/vars` 上以 JSON 格式公佈。通常它被用於伺服器操作計數。`helloRequests` 就是這樣一個 `int64` 變數，該處理函式對其加 1，然後寫入“hello world!”到瀏覽器。
...
計數器物件 `ctr` 有一個 `String()` 方法，所以它實現了 `expvar.Var` 介面。這使其可以被發佈，儘管它是一個結構體。`ServeHTTP()` 函式使 `ctr` 成為處理器，因為它的簽名正確實現了 `http.Handler` 介面。
...
該處理函式迭代 `os.Args` 以打印出所有的命令列引數。如果沒有指定則只有程式名稱（可執行程式的路徑）會被打印出來。
...
每當有新請求到達，通道的 `ServeHTTP()` 方法從通道獲取下一個整數並顯示。由此可見，網頁伺服器可以從通道中獲取要傳送的響應，它可以由另一個函式產生（甚至是客戶端）。下面的程式碼片段正是一個這樣的處理函式，但會在 30 秒後超時：
